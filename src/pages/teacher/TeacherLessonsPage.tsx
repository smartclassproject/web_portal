...
const TeacherLessonsPage: React.FC = () => {
  ...
  // Group lessons by course for chapter-style display (no schedule/date ordering)
  const lessonsByCourse = useMemo(() => {
    const map = new Map<string, Lesson[]>();
    for (const lesson of lessons) {
      const courseId = typeof lesson.courseId === 'object'
        ? (lesson.courseId as { _id: string })._id
        : lesson.courseId;
      if (!map.has(courseId)) map.set(courseId, []);
      map.get(courseId)!.push(lesson);
    }
    // Sort chapters within each course by title (chapter order)
    map.forEach((chapters) => {
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              Organize content by course and chapter. Add chapters to each course and attach materials and resources.
            </p>
          </div>
          <button
            onClick={() => {
        ...
        <div className="space-y-8">
          {courses.length === 0 ? (
            <div className="bg-white rounded-xl shadow-md p-8 text-center">
              <p className="text-gray-500">No courses yet. Assignments and courses are required to add chapters.</p>
            </div>
          ) : Array.from(lessonsByCourse.entries()).length === 0 ? (
            <div className="bg-white rounded-xl shadow-md overflow-hidden">
              <div className="px-6 py-12 text-center">
                ...
                <p className="mt-1 text-sm text-gray-500">Get started by adding a chapter to a course.</p>
                <div className="mt-6">
                  <button
                    onClick={() => {
                      setSelectedLesson(null);
            ...
            Array.from(lessonsByCourse.entries()).map(([courseId, chapterList]) => (
              <div key={courseId} className="bg-white rounded-xl shadow-md overflow-hidden">
                <div className="px-6 py-4 bg-gray-50 border-b border-gray-200">
                  <h2 className="text-lg font-semibold text-gray-900">{getCourseName(courseId)}</h2>
                  <p className="text-sm text-gray-500 mt-0.5">{chapterList.length} chapter{chapterList.length !== 1 ? 's' : ''}</p>
                </div>
