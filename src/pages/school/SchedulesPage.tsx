...
import type { Course, CreateScheduleData, Schedule, Teacher, WeeklySession } from '../../types';
...
const getCourseName = (courses: Course[], courseId: Course | string): string => {
...
const getTeacherName = (teachers: Teacher[], teacherId: Teacher | string): string => {
...
const getCalendarEvents = (schedules: Schedule[], courses: Course[], teachers: Teacher[]) => {
  const events: any[] = [];
...
const SchedulesPage: React.FC = () => {
  ...
  const filtered = schedules && schedules.filter(schedule => {
    const courseName = getCourseName(courses, schedule.courseId);
    const teacherName = getTeacherName(teachers, schedule.teacherId);
      ...
      <div className="max-w-6xl mx-auto space-y-8">
            ...
            >
              {loading ? (
                <svg className="animate-spin h-5 w-5" fill="none" viewBox="0 0 24 24">
                  ...
                  <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                </svg>
              ) : (
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex bg-gray-100 rounded-lg p-1">
              <button
                ...
                className={`px-3 py-1 rounded-md text-sm font-medium transition ${viewMode === 'table'
              ...
              <button
                ...
                className={`px-3 py-1 rounded-md text-sm font-medium transition ${viewMode === 'calendar'
                    ? 'bg-white text-blue-600 shadow-sm'
                    : 'text-gray-600 hover:text-gray-900'
                  }`}
              >
          ...
          <>
            ...
            <div className="bg-white rounded-xl shadow-md p-6 mb-6">
              <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                ...
                <div className="flex items-center gap-4">
                  <span className="text-sm font-medium text-gray-700">Time View:</span>
                  <div className="flex bg-gray-100 rounded-lg p-1">
                    <button
                      onClick={() => setTimeView('day')}
                      className={`px-3 py-1 rounded-md text-sm font-medium transition ${timeView === 'day'
                          ? 'bg-white text-blue-600 shadow-sm'
                          : 'text-gray-600 hover:text-gray-900'
                        }`}
                    >
                    ...
                    <button
                      ...
                      className={`px-3 py-1 rounded-md text-sm font-medium transition ${timeView === 'week'
                          ? 'bg-white text-blue-600 shadow-sm'
                          : 'text-gray-600 hover:text-gray-900'
                        }`}
                    >
                    ...
                    <button
                      ...
                      className={`px-3 py-1 rounded-md text-sm font-medium transition ${timeView === 'custom'
                          ? 'bg-white text-blue-600 shadow-sm'
                          : 'text-gray-600 hover:text-gray-900'
                        }`}
                    >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-center">
                    <div className="text-lg font-semibold text-gray-900">{formatDisplayDate()}</div>
                    <button
                      onClick={goToToday}
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2 mb-4">
              ...
              <div className="text-sm text-gray-600">
                {filtered && filtered.length} schedule{filtered && filtered.length !== 1 ? 's' : ''} found
              </div>
            </div>
            ...
            {loading ? (
              <div className="bg-white rounded-xl shadow-md p-12 text-center">
                ...
                <h3 className="text-lg font-medium text-gray-900 mb-2">Loading schedules...</h3>
              </div>
              ...
              <div className="bg-white rounded-xl shadow-md p-12 text-center">
                ...
                <p className="text-gray-500 mb-4">
                  {timeView === 'day' && `No schedules for ${format(selectedDate, 'EEEE, MMMM d, yyyy')}`}
                  {timeView === 'week' && `No schedules for the week of ${format(startOfWeek(selectedDate, { weekStartsOn: 1 }), 'MMM d, yyyy')}`}
                  {timeView === 'custom' && `No schedules between ${format(customStartDate, 'MMM d, yyyy')} and ${format(customEndDate, 'MMM d, yyyy')}`}
                </p>
                <button
                  onClick={() => setIsAddModalOpen(true)}
              ...
              <div className="overflow-x-auto bg-white rounded-xl shadow-md">
                <table className="min-w-full text-left">
                  ...
                  <tbody>
                    {filtered && filtered.map((schedule, idx) =>
                      schedule.weeklySessions.map((session, sessionIndex) => (
                        <tr key={`${schedule._id}-${sessionIndex}`} className="group border-b last:border-b-0 border-gray-100 hover:bg-blue-50 transition">
                          ...
                          <td className="py-3 px-4 text-gray-600">{session.day}</td>
        ...
        <AddScheduleModal
          ...
          initialData={selectedSchedule ? {
            courseId: typeof selectedSchedule.courseId === 'string' ? selectedSchedule.courseId : selectedSchedule.courseId._id,
            classroom: selectedSchedule.classroom,
            teacherId: typeof selectedSchedule.teacherId === 'string' ? selectedSchedule.teacherId : selectedSchedule.teacherId._id,
